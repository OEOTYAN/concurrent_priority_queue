...
public:
    ...
    : mark(0), my_compare(compare), c(begin, end, alloc) {
        my_aggregator.initialize_handler(functor{this});
        heapify();
...
private:
    ...
    void heapify() {
        ...
        for (; mark < c.size(); ++mark) {
            ...
            do { // push to_place up the heap
                ...
                if (!my_compare(c[parent], to_place))
                    break;
    ...
    void reheap() {
        ...
        while (child < mark) {
            ...
            if (child + 1 < mark && my_compare(c[child], c[child + 1]))
                ++target;
            // target now has the higher priority child
            if (my_compare(c[target], c.back()))
                break;
    ...
    alignas(std::hardware_destructive_interference_size) aggregator_type my_aggregator;
    // Padding added to avoid false sharing
    char padding1
        [std::hardware_destructive_interference_size -
         sizeof(aggregator_type) % std::hardware_destructive_interference_size];
    ...
    // Padding added to avoid false sharing
    char padding2
        [std::hardware_destructive_interference_size - (2 * sizeof(size_type)) -
         sizeof(Compare) % std::hardware_destructive_interference_size];
